 ...
 */
export async function runBatchedSync(opts: BatchedSyncOptions): Promise<SyncStatus> {
  ...
  try {
    ...
    const localLast =
      (opts.getLocalLastPageId ? opts.getLocalLastPageId() : null) ?? (await loadLastPageId());
    ...
    const res = await putMeState({ lastPageId: localLast ?? null, clientRev: localRev });
