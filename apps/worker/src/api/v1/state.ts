...
import {
	requireTelegramAuth,
	type AuthContextVars,
} from "../../security/tgAuth";
...
function toDto(
	...
	return {
		...
		clientRev: Number.isFinite(row.client_rev)
			? Math.max(0, Math.trunc(row.client_rev))
			: 0,
...
stateApi.put("/v1/me/state", requireTelegramAuth(), async (c) => {
	...
	const lastPageId =
		lastPageIdRaw === null ? null : String(lastPageIdRaw).trim();

	if (
		lastPageId !== null &&
		(lastPageId.length === 0 || lastPageId.length > 128)
	) {
	...
	const saved = await upsertStateIdempotent(
		c.env.DB,
		userId,
		lastPageId,
		Math.trunc(clientRev),
	);
