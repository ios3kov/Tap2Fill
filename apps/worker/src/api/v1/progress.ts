...
progressApi.put("/v1/progress/:pageId", requireTelegramAuth(), async (c) => {
	...
	const timeSpentSec = Number.isFinite(body.timeSpentSec)
		? Math.max(0, Math.floor(Number(body.timeSpentSec)))
		: 0;

	if (!contentHash)
		return c.json({ ok: false, error: "CONTENT_HASH_REQUIRED" }, 400);
	if (!Number.isInteger(clientRev) || clientRev < 0)
		return c.json({ ok: false, error: "CLIENT_REV_INVALID" }, 400);
	if (!dataB64 || !isBase64(dataB64))
		return c.json({ ok: false, error: "DATA_INVALID" }, 400);
